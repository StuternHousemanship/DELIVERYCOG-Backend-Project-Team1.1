...
export default class DeliveryRepository {
    ...
    async createDelivery(delivery: DeliveryType): Promise<DeliveryType> {
        const { userId, item, destination, sender_name, sender_number, reciever_name, reciever_number } = delivery;
        const newDelivery: any = await Delivery.query()
            .insert({
    ...
    async getAllDelivery(userId: string | number): Promise<DeliveryType[]> {
        const delivery: DeliveryType[] = (await Delivery.query().where('userId', userId).then(rows => rows)) as DeliveryType[];

        return delivery ;
    }
    async getDeliveryById(orderId: string | number, userId: string | number): Promise<DeliveryType[]> {
